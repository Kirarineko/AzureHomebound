...
本 skill 内的相对路径均以本 skill 目录(调用 skill 工具加载时注入的 "Base directory for this skill")为基准:

- `references/start.md` = `<基准>/references/start.md`
- `scripts/scan_tasks.py` = `<基准>/scripts/scan_tasks.py`
- `references/` 内文件间的链接(如 start.md 里的 `init.md`)以 `references/` 目录为基准; `../assets/` 即 `<基准>/assets/`.

若 skill 工具不可用而直接 Read 了文件: 项目安装时基准 = `.opencode/skills/whatsnext/`, 全局安装时 = `~/.config/opencode/skills/whatsnext/`.

## 统一前置: 装载契约(幂等)

所有 `/wn*` 命令的第一步都是确保本契约已在上下文, 再进入各自动作:

- opencode 的 skill 加载是显式的: 调用 skill 工具后本 `SKILL.md` 正文即入上下文. "已装载" = 本正文已在当前会话上下文里(本会话早先跑过某个 `/wn*` 加载过).
- 判断靠自省"这段契约现在在我上下文吗". 是软信号, **不确定时就读**——重读本文件幂等, 代价仅几百 token(本 skill 位于项目内或全局配置目录, 读取无需额外授权).
- 已装载则直接进下一步, 不重复装载.
...
.whatsnext/
  ...
  # scripts 随插件分发于本 skill 的 scripts/, 非计划区内容
```

- **无根索引**: `tasks/` 下不放手写索引文件. 真相唯一来源是各任务 `index.md` 的 frontmatter, 由 `scan_tasks.py` 扫描汇总(任务列表 + Focus). 消除"手写台账与磁盘漂移".
- **任务快照脚本**: `python3 scripts/scan_tasks.py` 输出 `{tasks:[{dir,status,title}], focus:string[]}`, 支持 `--status` / `--tags` 筛选. 列任务 / 找 Focus / 对账都调它, 免逐个读文件; 无 python 时回退直接遍历 frontmatter.
...
1. 调 `scripts/scan_tasks.py` → 拿任务快照 + Focus(`focus` 数组: 空=无, 单=聚焦, 多=冲突)
...
2. 调 `python3 scripts/search_knowledge.py` 扫 `.whatsnext/knowledge/` 各文件 frontmatter(输出 `{knowledge:[{name,title,label,description}]}`, 支持 `--label` 筛 / `--tags` 模糊匹配), 与用户请求 + 任务目标比对; 命中候选后再在文件内按需搜正文. 无 python 时回退直接扫 frontmatter.
